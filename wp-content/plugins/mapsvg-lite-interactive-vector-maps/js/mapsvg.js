...
(function($, window, MapSVG, Math){
    ...
    MapSVG.DirectoryController.prototype.setEventHandlers = function(){
        ...
        this.view.on('click.menu.mapsvg', '.mapsvg-directory-item', function (e) {
            ...
            if(!_data.editMode && actions.directoryItem.click.showAnotherMap){
                ...
                if(linkParts.length > 1){
                    ...
                    if(map_id){
                        var container = !MapSVG.parseBoolean(actions.directoryItem.click.showAnotherMapSameContainer) ? $('#'+actions.directoryItem.click.showAnotherMapContainerId) : _data.$map;
...
(function( $ ) {
    ...
    var defaults = {
        ...
        responsive          : true,
        tooltips            : {on: false, position: 'bottom-right', template: ''},
        ...
        this.methods = {
            ...
            setLayersControl : function(options){
                ...
                if(_data.options.layersControl.on){
                    ...
                    if(!_data.editMode && _data.options.menu.on && !_data.options.menu.customContainer && _data.options.layersControl.position.indexOf('left')!==-1){
            ...
            setMenu : function(options){
                ...
                if(_data.options.menu.on && !_data.editMode){
                    ...
                    _data.controllers.directory = new MapSVG.DirectoryController({
                        ...
                        scrollable: !_data.options.menu.customContainer,
            ...
            setDatabase : function(options){
                ...
                if(options.pagination && options.pagination.on != undefined)
                    ...
                    if(!MapSVG.googleMapsApiLoaded){
                        ...
                        if(!_data.googleMaps.map){
                            ...
                            USGSOverlay.prototype.draw = function() {
                                ...
                                // console.log('NOW sw:'+coords.sw.x+' and ne:'+coords.ne.x+'and W-width:'+ww+' scale:'+scale);
                                console.log('NOW sw2:'+coords.sw2.x+' and ne2:'+coords.ne2.x+'and scale:'+scale+' and ww='+ww);
            ...
            selectRegion :    function(id, skipDirectorySelection){
                ...
                if(_data.options.multiSelect && !_data.editRegions.on){
                    if(region.selected){
                        _this.deselectRegion(region);
                        if(!skipDirectorySelection && _data.options.menu.on && !_data.editMode){
            ...
            regionClickHandler : function(e, region, skipPopover){
                ...
                if(region.mapsvg_type == 'region'){
                    ...
                    if(!_data.editMode && actions.region.click.showDetails){
                        _this.loadDetailsView(region);
                    }

                    if(actions.region.click.showPopover){
                        _this.showPopover(region);
                    }else if(e.type.indexOf('touch')!==-1 && actions.region.touch.showPopover){
                        _this.showPopover(region);
                    }

                    if(!_data.editMode && actions.region.click.goToLink){
                    ...
                    if(!_data.editMode && actions.region.click.showAnotherMap){
                        ...
                        if(linkParts.length > 1){
                            ...
                            if(map_id){
                                var container = !MapSVG.parseBoolean(actions.region.click.showAnotherMapSameContainer) ? $('#'+actions.region.click.showAnotherMapContainerId) : _data.$map;
                                jQuery.get(ajaxurl, {action:"mapsvg_get",id: map_id},function(data){
                                    if(container.find('svg').length)
                                        container.mapSvg().destroy(); 
                                    eval('var options = '+data);
                    ...
                    var passingObject = region.databaseObject;

                    if((_data.editMode || _data.options.menu.on) && actions.marker.click.filterDirectory)
                        _this.database.getAll({filters: {id: region.databaseObject.id}});

                    if(!_data.editMode && actions.marker.click.showDetails)
                        _this.loadDetailsView(passingObject);
                    if(actions.marker.click.showPopover){
                        _this.showPopover(passingObject);
                    }else if(e.type.indexOf('touch')!==-1 && actions.marker.touch.showPopover){
                        _this.showPopover(passingObject);
                    }
                    if(!_data.editMode && actions.marker.click.goToLink){
            ...
            getPopoverBody: function(object){
                var name = 'popover'+MapSVG.ucfirst(region.mapsvg_type);
                return _data.templates[name](object);
            },
            ...
            init: function(opts, elem) {
                ...
                $.ajax({url: _data.options.source+'?v='+_data.options.svgFileVersion})
                    .done(function(xmlData){
                        ...
                        // Don't show default browser "title" tooltips
                        // $('path["title"]').on('mouseenter',function(e){
                        //    e.preventDefault();
                        // });
                        ...
                        _this.regionsDatabase.on('dataLoaded', function(){
                            _this.loadDirectory();
                            _data.events['regionsLoaded'] && _data.events['regionsLoaded'].call(_this);
                        });

                        _data.menuDatabase = _data.options.menu.source == 'regions' ? _this.regionsDatabase : _this.database;
                        _this.setMenu();

                        if(!_data.editMode && _data.options.menu.filterout.field){
                            var f = {};
                        ...
                        _this.database.getAll().done(function (data) {
                            _this.regionsDatabase.getAll().done(function(regions){
                                _this.regionsDatabase.loadSchema().done(function(){
